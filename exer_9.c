/*
Um clube de viagens necessita gerir os seus sócios.
Faça um programa em C que deve permitir:
-Inserir sócios: número e nome
-Inserir destino de viagens
-Inscrever sócios em viagens
-Listar sócios por número: nome, número e destino de viagens
- Listar viagens por nome de destino: nome, número e nome de sócios inscritos nesse destino

*/

// IMPOPRTACAO DE BIBLIOTECAS 
#include <stdio.h>
#include <string.h>
...
//ESTRUTURA REGISTO VIAGEM PESSOA 
typedef struct{
...
//ESTRUTURA REGISTO PESSOA VIAGEM  
typedef struct{
...
void EscreveTodosPessoas(P* x, V* y,  RVP* VP) {  
    ...
    // INDICES PARA PERCORRER AS LISTAS
    int i,queviagens,LiVPqueviagens;
    
    // INICIALIZA O I COM O ELO PARA O PRIMEIRO SOCIO REAL
    i=x[0].elo;
    // CICLO QUE PERCORRE AS PESSOAS 
    while ( i != 0 )
    {
        if (x[i].PrimPV==0)
            ...
            // EH PARA PERCORRER TODAS AS VIAGENS DE CADA PESSOA
            // TENHO Q PERCORRER A LISTA DE VIAGENS PESSOA E IMPRIMIR AQUELAS Q TÊM AQUELA PESSOA NESTE CASO O LIMITE SERIAM 30 VIAGENS 
            while (queviagens!=0)
            {
...
void EscreveTodosviagens(P* x, V* y, RPV* PV) {     
    ...
    // VAIS PERCORRER A LISTA DE VIAGENS COM O INDICE I
    int i;
    int temp;
    int liVPtemp;
    
    // APONTA PARA A PRIMEIRA VIAGEM REAL
    i=y[0].elo;
    
    while (i!=0)
    {
        // TEMP GUARDA A PRIMEIRA PESSOA DA VIAGEM
        temp=y[i].PrimVP;
        ...
        {
            // IMPRIMIR CADA PESSOA DA VIAGEM
...
P LeUm() {
    ...
    do {
        printf("\nQual o n?mero do pessoa a inserir?");
        scanf("%d",&um.num);
...
int main ()
{
    // DECLARACAO DO ARRAY DE SOCIOS COM 10 POSICOES
    P x[10];  
    
    // DECLARACAO DE UMA STRUCT SOCIO
    P um;
    
    // DECLARACAO DO ARRAY DE VIAGENS COM 10 POSICOES
    V y[10];
    
     // DECLARACAO DE UMA STRUCT VIAGEM 
    V dois;
    
     // DECLARACAO DO ARRAY PESSOAvIAGEM  COM 30 POSICOES
    RPV PV[30];
    
    // DECLARACAO DO ARRAY VIAGEMPESSOA  COM 30 POSICOES
    RVP VP[30];

    // DECLARAR VARIAVEL PARA GUARDAR A OPCAO
    int op;
    ...
    // ENQUANTO A OPCAO FOR DIFERENTE DE 6
    while ( op != 6 )
    {
    	switch (op) {
